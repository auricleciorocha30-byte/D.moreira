    ...
    <div className="w-full">
            ...
            <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] shadow-xl border border-gray-100 flex flex-col h-auto min-h-[600px]">
              ...
              <div className="bg-yellow-50 p-6 rounded-[2rem] border-2 border-yellow-100 mb-8 space-y-4">
                ...
                {loyalty.scopeType !== 'all' && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto no-scrollbar p-2 bg-white/50 rounded-xl border border-yellow-100/50">
                    {loyalty.scopeType === 'category' ? categories?.map(cat => (
                      <button key={cat.id} onClick={() => toggleLoyaltyItem(cat.name)} className={`p-2 rounded-lg border-2 text-[8px] font-black uppercase transition-all ${loyaltyScopeItems.includes(cat.name) ? 'bg-yellow-400 border-black shadow-sm' : 'bg-white border-transparent text-gray-400 hover:border-gray-200'}`}>
                        {cat.name}
                      </button>
                    )) : menuItems?.map(prod => (
                      <button key={prod.id} onClick={() => toggleLoyaltyItem(prod.id)} className={`p-2 rounded-lg border-2 text-[8px] font-black uppercase transition-all ${loyaltyScopeItems.includes(prod.id) ? 'bg-yellow-400 border-black shadow-sm' : 'bg-white border-transparent text-gray-400 hover:border-gray-200'}`}>
                        {prod.name}
                      </button>
                    ))}
                  </div>
      ...
      {selectedTable && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-7xl h-[92vh] rounded-[3rem] flex flex-col overflow-hidden shadow-2xl border-t-8 border-yellow-400 animate-in zoom-in duration-300">
            {/* Header Fixo */}
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden min-h-0 bg-gray-50">
               ...
               <div className="flex-1 p-6 md:overflow-y-auto space-y-6 bg-white border-b md:border-b-0 min-w-0">
                  <div className="bg-yellow-50 p-6 rounded-[2.5rem] border-2 border-yellow-100 shadow-sm md:sticky md:top-0 md:z-20">
                    <div className="flex justify-between items-start text-black">
                      <div className="flex flex-col min-w-0">
                        <p className="text-[8px] font-black text-yellow-700 uppercase tracking-widest">Cliente</p>
                        <p className="font-black text-lg uppercase tracking-tight truncate">{selectedTable.currentOrder?.customerName}</p>
                      </div>
               ...
               <div className="w-full md:w-80 lg:w-96 bg-gray-50 p-6 md:border-l md:overflow-y-auto shrink-0 shadow-[inset_10px_0_15px_-10px_rgba(0,0,0,0.05)]">
                  ...
                  <div className="space-y-4 mb-6">
                    <input type="text" value={productSearchForTable} onChange={e => setProductSearchForTable(e.target.value)} placeholder="BUSCAR PRODUTO..." className="w-full bg-white border-2 rounded-2xl px-5 py-3 text-[10px] font-black outline-none uppercase focus:border-yellow-400 transition-all shadow-sm" />
                    <textarea value={currentObservation} onChange={e => setCurrentObservation(e.target.value)} placeholder="OBSERVAÇÃO OPCIONAL..." className="w-full bg-white border-2 rounded-2xl px-5 py-3 text-[9px] font-black outline-none uppercase h-20 resize-none focus:border-yellow-400 transition-all shadow-sm" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-2 pb-10">
                    {filteredProductsForTable.map(p => (
                      <button key={p.id} onClick={() => { onAddToOrder(selectedTable.id, p, currentObservation); setCurrentObservation(''); }} className="w-full bg-white p-3 rounded-2xl border border-gray-100 flex gap-3 items-center hover:border-black transition-all active:scale-[0.98] shadow-sm group">
                        ...
                        <span className="bg-yellow-400 text-black w-7 h-7 rounded-lg flex items-center justify-center font-black text-lg shrink-0 shadow-sm group-hover:bg-black group-hover:text-yellow-400 transition-colors">+</span>
                      </button>
                    ))}
              ...
              {couponForm.scopeType !== 'all' && (
                <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto no-scrollbar p-2 bg-gray-50 rounded-xl">
                  {couponForm.scopeType === 'category' ? categories.map(cat => (
                    <button key={cat.id} type="button" onClick={() => toggleCouponItem(cat.name)} className={`p-2 rounded-lg border-2 text-[8px] font-black uppercase transition-all ${couponForm.selectedItems.includes(cat.name) ? 'bg-yellow-400 border-black shadow-sm' : 'bg-white border-transparent hover:border-gray-300'}`}>{cat.name}</button>
                  )) : menuItems.map(prod => (
                    <button key={prod.id} type="button" onClick={() => toggleCouponItem(prod.id)} className={`p-2 rounded-lg border-2 text-[8px] font-black uppercase transition-all ${couponForm.selectedItems.includes(prod.id) ? 'bg-yellow-400 border-black shadow-sm' : 'bg-white border-transparent hover:border-gray-300'}`}>{prod.name}</button>
                  ))}
                </div>
              )}
