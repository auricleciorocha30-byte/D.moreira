                ...
                <>
                  <div className="bg-gray-50 p-6 rounded-[2.5rem] border border-gray-100 space-y-6 font-black uppercase shadow-sm">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="SEU NOME" className="w-full bg-white border-2 rounded-2xl px-6 py-4 text-xs outline-none shadow-sm focus:border-yellow-400 transition-all"/>
                      <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="WHATSAPP (PARA FIDELIDADE)" className="w-full bg-white border-2 rounded-2xl px-6 py-4 text-xs outline-none shadow-sm focus:border-yellow-400 transition-all"/>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {storeConfig.tablesEnabled && (
                          <button onClick={() => setOrderType('table')} className={`py-3.5 rounded-2xl text-[8px] border-2 transition-all ${orderType === 'table' ? 'bg-black text-white border-black shadow-md' : 'bg-white text-gray-400 border-gray-100'}`}>Na Mesa</button>
                        )}
                        {storeConfig.counterEnabled && (
                          <button onClick={() => setOrderType('takeaway')} className={`py-3.5 rounded-2xl text-[8px] border-2 transition-all ${orderType === 'takeaway' ? 'bg-black text-white border-black shadow-md' : 'bg-white text-gray-400 border-gray-100'}`}>Balcão</button>
                        )}
                        {storeConfig.deliveryEnabled && (
                          <button onClick={() => setOrderType('delivery')} className={`py-3.5 rounded-2xl text-[8px] border-2 transition-all ${orderType === 'delivery' ? 'bg-black text-white border-black shadow-md' : 'bg-white text-gray-400 border-gray-100'}`}>Entrega</button>
                        )}
                      </div>
                    </div>
                    ...
                    {orderType === 'table' && (
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 gap-2">{Array.from({ length: 12 }, (_, i) => i + 1).map(num => (
                          <button key={num} onClick={() => setTableNumber(num.toString())} className={`py-2.5 rounded-xl text-xs transition-all ${tableNumber === num.toString() ? 'bg-yellow-400 text-black border-yellow-500 shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}>{num}</button>
                        ))}</div>
                      </div>
                    )}
                    ...
                    {orderType === 'delivery' && (
                      <div className="space-y-2">
                        ...
                        <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="RUA, NÚMERO, BAIRRO E COMPLEMENTO" className="w-full bg-white border-2 rounded-2xl px-6 py-4 text-xs outline-none h-24 resize-none shadow-sm focus:border-yellow-400 transition-all"/>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-1.5">{(['Pix', 'Dinheiro', 'Cartão'] as const).map(method => (
                        <button key={method} onClick={() => setPaymentMethod(method)} className={`py-3.5 rounded-2xl text-[9px] border-2 transition-all ${paymentMethod === method ? 'bg-black text-white border-black shadow-md' : 'bg-white text-gray-400 border-gray-100'}`}>{method}</button>
                      ))}</div>
                    </div>

                    <div className="space-y-2">
                      <p className="text-[8px] tracking-[0.2em] text-gray-400 ml-2">Observações</p>
                      <textarea value={observation} onChange={(e) => setObservation(e.target.value)} placeholder="EX: SEM CEBOLA, CAPRICHA NO MOLHO..." className="w-full bg-white border-2 rounded-2xl px-6 py-4 text-xs outline-none h-20 resize-none shadow-sm focus:border-yellow-400 transition-all"/>
                    </div>

                    <div className="flex gap-3 pt-2">
                      <input type="text" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} placeholder="TEM UM CUPOM?" className="flex-1 bg-white border-2 rounded-2xl px-6 py-4 text-[10px] outline-none shadow-sm focus:border-yellow-400 transition-all uppercase"/>
                      <button onClick={handleValidateCoupon} className="bg-black text-yellow-400 px-6 py-4 rounded-2xl text-[10px] shadow-lg active:scale-95 transition-all">Validar</button>
                    </div>
                    ...
                    {items.map(item => (
                      <div key={item.id} className="flex gap-4 bg-white p-4 rounded-[2rem] border items-center relative group shadow-sm hover:border-black transition-all overflow-hidden">
                        ...
                        <div className="flex-1 font-black min-w-0 pr-8">
                          ...
                          <p className="text-yellow-700 text-xs italic">R$ {item.price.toFixed(2)}</p>
            ...
            {items.length > 0 && (
              <div className="p-8 md:p-10 border-t-4 bg-white sticky bottom-0 rounded-t-[3.5rem] z-20 font-black italic shadow-[0_-20px_50px_-12px_rgba(0,0,0,0.15)]">
                ...
                <button 
                  ...
                  className="w-full bg-yellow-400 text-black py-6 rounded-[2.5rem] uppercase text-[11px] font-black tracking-widest shadow-xl active:scale-95 transition-all hover:brightness-110 flex items-center justify-center gap-3"
                >
